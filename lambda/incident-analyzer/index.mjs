...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!description) return { statusCode: 400, headers, body: JSON.stringify({ error: 'description is required' }) };
    if (description.length > 800) return { statusCode: 400, headers, body: JSON.stringify({ error: 'description must be 800 characters or fewer' }) };
    return { statusCode: 200, headers, body: JSON.stringify(classifyIncident(description)) };
  } catch (error) {
    console.error('SupportShield error', error);
